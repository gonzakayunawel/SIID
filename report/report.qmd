...
author: 
  - name: "Gonzalo Cayunao Erices"
    affiliation: 
      - "Sistemas de Información e Inteligencia de los Datos"
      - "Doctorado en Ingeniería de Sistemas Avanzados"
      - "Universidad Andrés Bello (CL), 2026"
date: today
format:
  html:
    toc: true
  ...
  pdf:
    ...
    - text: |
        ...
        \renewcommand\@maketitle{%
            ...
            \vspace{0.3em}
            {\raggedright\small\itshape Sistemas de Información e Inteligencia de los Datos\par}
            {\raggedright\small\itshape Doctorado en Ingeniería de Sistemas Avanzados\par}
            {\raggedright\small\itshape Universidad Andrés Bello (CL), 2026\par}
            \vspace{0.5em}
            {\raggedright\small\@date\par}
            \vspace{2em}
        }
        \makeatother
execute:
...
Este informe técnico documenta la transformación de datos crudos en inteligencia operativa para la gestión de la cadena de suministros (Supply Chain Managment). Utilizando técnicas avanzadas de **Ciencia de Datos**, se ha analizado el ecosistema de **DataCo Global** para diagnosticar ineficiencias logísticas.

Los resultados demuestran que, a través de modelos de clasificación como **Logistic Regression** o **XGBoost**, es posible anticipar interrupciones en la entrega con una precisión superior al **95%**. Por otra lado, con el uso de modelos como **Linear Regression**, podemos hacer una previsión de las ventas con un R^2^ igual a 1. 

El análisis se estructura en cuatro pilares: contextualización del negocio y calidad de datos, diagnóstico exploratorio con análisis probabilístico, benchmarking de modelos predictivos y recomendaciones de optimización estratégica.
...
2. **Modelado Predictivo**: Implementar un benchmarking de algoritmos de clasificación para predecir `Late_delivery_risk` y un benchmarking de algoritmos de regresión para predecir `Sales`.
...
# Convertir a datetime para usar DateFormatter
monthly_sales["order_date"] = pd.to_datetime(monthly_sales["order_month"])
...
ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %y"))  # Ene 15, Feb 15
...
sns.barplot(
    x=status_pct.values,
    y=status_pct.index,
    hue=status_pct.index,
    palette="muted",
    legend=False,
)
plt.title("Estado de Entregas — Proporción (%)")
plt.xlabel("Porcentaje (%)")
plt.ylabel("Estado de entrega")
plt.tight_layout()
plt.show()
```
...
importances = pd.Series(
    dt_ref.named_steps["model"].feature_importances_, index=feature_cols_clf
...
display(
    importances.head(5).rename("Importancia (Gini)").to_frame().style.format("{:.4f}")
)
```
...
## Selección del Problema: Late Delivery Risk y Sales
...
Por otra parte, las ventas (`Sales`) son el motor financiero que genera ingresos y rentabilidad empresarial, permitiendo reinversión en operaciones, innovación y crecimiento. En **supply chain**, optimizar ventas mediante entregas puntuales, reducción de costos y satisfacción del cliente es fundamental para la sostenibilidad y competitividad del negocio.
...
Benchmarking de cuatro modelos de clasificación para predecir `Late_delivery_risk` a partir de variables con mayor importancia.
...
### Métricas por Modelo y Curva ROC
